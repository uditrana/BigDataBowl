...
plays = list(set(map(lambda x: (x[0].item(), x[1].item()), track_df.groupby(
...
#             L_given_t = L_given_ts[t_i, clipped_reach_vecs[1]-y_min, clipped_reach_vecs[0]-x_min] * L_t_mask  # (F,) ; index with y and then x
            ...
            T_given_L_subset = T_given_Ls_df.set_index('dist').loc[reach_dist_in_bounds, 'p'].to_numpy(dtype=dt)                .reshape(
                len(reach_dist_in_bounds), -1)  # (F~, T) ; F~ is subset of F that is in [1, 60] yds from ball
        ...
        def comp_prob():
            ...
            # F, T, T # maybe change this to a normal distribution
            lambda_z = np.where((traj_locs_z < 3) & (traj_locs_z > 0), 1, 0)
    ...
    #         return field_df
        ...
        def xyac():
            receivers_df = frame_df.loc[frame_df.team_pos == 'DEF', ['x_r', 'y_r',
                                                                     'v_x_r', 'v_y_r', 'v_r_theta', 'v_r_mag', 'los', 'a_x', 'a_y']]
...
#                                                              receivers_df.y - ball_start[1]), axis=0)
    ...
    #         y_r = rec_y_np + rec_v_y_np*params.reax_t + 0.5*rec_a_y_np*params.reax_t**2 #(J, 1)
            ...
            int_s0 = np.clip(np.sum(int_d_vec*reaction_player_vels, axis=2)/int_d_mag, -params.s_max,
                             params.s_max)  # F, J,  #taking norm between vectors int_d and player velocity
            ...
            a1 = np.broadcast_to(t_lt_smax[:, :, None], (*t_lt_smax.shape, len(T))
                                 )  # F, J, T THIS IS TIME SPENT LT SMAX
            ...
            d = time_at_smax * params.s_max + int_s0[:, :,
                                                     None]*time_lt_smax + 0.5*params.a_max*np.square(time_lt_smax)  # F, J, T
            ...
            just_top_5_distances = np.reshape(
                ...
                (just_top_5_distances.shape[0] * just_top_5_distances.shape[1],
                 just_top_5_distances.shape[2]))
...
for (gid, pid) in tqdm(random.sample(plays, 25)):
    print(gid, pid)
    dt = np.float32
    try:
        play_eppa(gid, pid, viz_df=False, save_np=True)
